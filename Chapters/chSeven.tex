...
\paragraph{} "Oh they aren't anyone important," Gus said to Snow. "We are trying to find the golden goose, and it seems like you are heading in the same direction as us. Mind if we join you?"
...
{All the pairs that the Geese could have formed were \{(Gus, Duckie), (Gus, Barnie), (Gus, Swan), (Snow, Duckie), (Snow, Barnie), (Snow, Swan), (Grey, Duckie), (Grey, Barnie), (Grey, Swan), (Blue, Duckie), (Blue, Barnie), (Blue, Swan)\}. There were 4 geese with red feet, and 3 geese with blue feet. There were \opmul{4}{3} ways to form pairs}
...
{Maggie and Gus can quite a few sets. These include \{\}, the $AxB$ itself, all sets with only a single pair, all the sets with only two pairs, etc.}
...
{As the dance began, Maggie watched the other geese dance. Gus noticed her watching, and came over. 
...
{The new pairing was \{(Maggie, Gus), (Barnie, Snow), (Swan, Grey), (Duckie, Blue)\}. This new pairing was also a function, because every first element only mapped to a single output. For example, originally, $f($Gus$)= $Maggie, but now $f^{-1}($Maggie$) = $Gus}
